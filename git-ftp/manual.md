...
Git-ftp - это FTP-клиент с использованием [Git], чтобы определить, какие локальные файлы загружать или
которые файлы удаляются на удаленном хосте.

Он сохраняет развернутое состояние, загружая хэш SHA1 в `.git-ftp.log`
файл. Git не требуется устанавливать на удаленном хосте.

Даже если вы работаете с разными ветвями, git-ftp знает, какие файлы
разные и обрабатывает только те файлы. Это экономит время и пропускную способность.

# ДЕЙСТВИЯ

`init`
:	загружает все необработанные файлы без отслеживания на удаленный сервер и
	создает файл `.git-ftp.log`, содержащий SHA1 последнего
	commit.

`catchup` 
:	Создает или обновляет файл `.git-ftp.log` на удаленном хосте.
	Предполагается, что вы уже загрузили все остальные файлы.
	Возможно, вы сделали это с другой программой.

`push`
:	Загружает файлы, которые изменились и
 	удаляет файлы, которые были удалены с момента последней загрузки.

`download` (ЭКСПЕРИМЕНТАЛЬНЫЙ)
:	Загружает изменения с удаленного хоста в рабочее дерево.
	Эта функция требует установки lftp и не использует никакой мощности
	Git.
	ПРЕДУПРЕЖДЕНИЕ. Он может удалять локальные невоспроизводимые файлы, которые не указаны в
	вашем файле `.git-ftp-ignore`.

`pull` (ЭКСПЕРИМЕНТАЛЬНЫЙ)
:	Загружает изменения с удаленного узла в отдельную фиксацию
	и сливает это в вашу текущую ветку.
	Эта функция должна быть установлена lftp.
